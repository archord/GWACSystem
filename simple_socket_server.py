...
class SimpleSocketServer(EventBus, metaclass=_Singleton):
    ...
    def run(self, host='0.0.0.0', port=6666, max_conn=5):
        ...
        while True:
            ...
            ttime = end-start
            if ttime>5:
                print("loop %d times, use %.2f s"%(tcount, ttime))
                start = time.perf_counter()
                tcount = 0
                self.__check_status()
    ...
    def __check_status(self):
        ...
        if len(self.__inputs)>1:
            gwacQuery = GWACQuery()
            dbrows = gwacQuery.queryFwhmRecordsFromUpdate()
            print("query %d rows"%(len(dbrows)))
        print("total has %d inputs, %d clients"%(len(self.__inputs), len(self.__clients)))
        for sock in self.__inputs:
            if sock in self.__clients:
                print(self.__clients[sock]) #('127.0.0.1', 57644)
                focusMessage = "fwhm:2.5\r\n"
                self.emit('onfocus', sock, self.__clients[sock], dbrows)
